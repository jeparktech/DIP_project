...
Copyright (c) 2006-2020 Tampere University.
All rights reserved.
...
def _process_psd(sigma_psd: Union[np.ndarray, float], z: np.ndarray,
    ...
    if isinstance(profile.lambda_thr3d, float):
        profile.lambda_thr3d = [profile.lambda_thr3d]
    if isinstance(profile.mu2, float):
        profile.mu2 = [profile.mu2]
    if isinstance(profile.lambda_thr3d_re, float):
        profile.lambda_thr3d_re = [profile.lambda_thr3d_re]
    if isinstance(profile.mu2_re, float):
        profile.mu2_re = [profile.mu2_re]

    profile.lambda_thr3d = [profile.filter_strength * i for i in profile.lambda_thr3d]
    profile.mu2 = [profile.filter_strength ** 2 * i for i in profile.mu2]
    profile.lambda_thr3d_re = [profile.filter_strength * i for i in profile.lambda_thr3d_re]
    profile.mu2_re = [profile.filter_strength ** 2 * i for i in profile.mu2_re]
